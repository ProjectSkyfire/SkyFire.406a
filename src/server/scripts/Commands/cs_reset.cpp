...
public:
    ...
    static bool HandleResetSpellsCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
            ...
            ChatHandler(target->GetSession()).SendSysMessage(LANGUAGE_RESET_SPELLS);
            if (!handler->GetSession() || handler->GetSession()->GetPlayer() != target)
                handler->PSendSysMessage(LANGUAGE_RESET_SPELLS_ONLINE, handler->GetNameLink(target).c_str());
        }
        else
        {
            ...
            handler->PSendSysMessage(LANGUAGE_RESET_SPELLS_OFFLINE, targetName.c_str());
        }
    ...
    static bool HandleResetTalentsCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!handler->extractPlayerTarget((char*)args, &target, &targetGuid, &targetName))
        {
            ...
            if (!*args && creature && creature->isPet())
            {
                ...
                if (owner && owner->GetTypeId() == TYPEID_PLAYER && creature->ToPet()->IsPermanentPetFor(owner->ToPlayer()))
                {
                    ...
                    ChatHandler(owner->ToPlayer()->GetSession()).SendSysMessage(LANGUAGE_RESET_PET_TALENTS);
                    if (!handler->GetSession() || handler->GetSession()->GetPlayer() != owner->ToPlayer())
                        handler->PSendSysMessage(LANGUAGE_RESET_PET_TALENTS_ONLINE, handler->GetNameLink(owner->ToPlayer()).c_str());
                }
                return true;
            }
        ...
        if (target)
        {
            ...
            ChatHandler(target->GetSession()).SendSysMessage(LANGUAGE_RESET_TALENTS);
            if (!handler->GetSession() || handler->GetSession()->GetPlayer() != target)
                handler->PSendSysMessage(LANGUAGE_RESET_TALENTS_ONLINE, handler->GetNameLink(target).c_str());
        ...
        else if (targetGuid)
        {
            ...
            handler->PSendSysMessage(LANGUAGE_RESET_TALENTS_OFFLINE, nameLink.c_str());
            return true;
        }
    ...
    static bool HandleResetAllCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (caseName == "spells")
        {
            ...
            if (!handler->GetSession())
                handler->SendSysMessage(LANGUAGE_RESETALL_SPELLS);
        }
        else if (caseName == "talents")
        {
            ...
            if (!handler->GetSession())
               handler->SendSysMessage(LANGUAGE_RESETALL_TALENTS);
        }
        else
        {
            handler->PSendSysMessage(LANGUAGE_RESETALL_UNKNOWN_CASE, args);
            handler->SetSentErrorMessage(true);
            return false;
        }
