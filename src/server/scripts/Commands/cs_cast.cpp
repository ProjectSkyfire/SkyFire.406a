...
public:
    ...
    static bool HandleCastCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!spellInfo)
        {
            handler->PSendSysMessage(LANGUAGE_COMMAND_NOSPELLFOUND);
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->PSendSysMessage(LANGUAGE_COMMAND_SPELL_BROKEN, spellId);
            handler->SetSentErrorMessage(true);
            return false;
        }
    ...
    static bool HandleCastTargetCommad(ChatHandler* handler, char const* args)
    {
        ...
        if (!caster->getVictim())
        {
            handler->SendSysMessage(LANGUAGE_SELECTED_TARGET_NOT_HAVE_VICTIM);
            handler->SetSentErrorMessage(true);
            return false;
        }
