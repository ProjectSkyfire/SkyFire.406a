...
public:
        ...
        switch (sWorld->BanCharacter(name, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName() : ""))
        {
            case BAN_SUCCESS:
            {
                if (atoi(durationStr) > 0)
                    handler->PSendSysMessage(LANGUAGE_BAN_YOUBANNED, name.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
                else
                    handler->PSendSysMessage(LANGUAGE_BAN_YOUPERMBANNED, name.c_str(), reasonStr);
                break;
            }
            case BAN_NOTFOUND:
            {
                handler->PSendSysMessage(LANGUAGE_BAN_NOTFOUND, "character", name.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }
            default:
    ...
    static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        ...
        switch (mode)
        {
            case BAN_ACCOUNT:
                ...
                {
                    handler->PSendSysMessage(LANGUAGE_ACCOUNT_NOT_EXIST, nameOrIP.c_str());
                    handler->SetSentErrorMessage(true);
                    return false;
                }
                break;
        ...
        switch (sWorld->BanAccount(mode, nameOrIP, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName() : ""))
        {
            case BAN_SUCCESS:
                if (atoi(durationStr) > 0)
                    handler->PSendSysMessage(LANGUAGE_BAN_YOUBANNED, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
                else
                    handler->PSendSysMessage(LANGUAGE_BAN_YOUPERMBANNED, nameOrIP.c_str(), reasonStr);
                break;
            ...
            case BAN_NOTFOUND:
                ...
                {
                    default:
                        handler->PSendSysMessage(LANGUAGE_BAN_NOTFOUND, "account", nameOrIP.c_str());
                        break;
                    case BAN_CHARACTER:
                        handler->PSendSysMessage(LANGUAGE_BAN_NOTFOUND, "character", nameOrIP.c_str());
                        break;
                    case BAN_IP:
                        handler->PSendSysMessage(LANGUAGE_BAN_NOTFOUND, "ip", nameOrIP.c_str());
                        break;
                }
                handler->SetSentErrorMessage(true);
                return false;
    ...
    static bool HandleBanInfoHelper(uint32 accountId, char const* accountName, ChatHandler* handler)
    {
        ...
        if (!result)
        {
            handler->PSendSysMessage(LANGUAGE_BANINFO_NOACCOUNTBAN, accountName);
            return true;
        }

        handler->PSendSysMessage(LANGUAGE_BANINFO_BANHISTORY, accountName);
        do
        {
            ...
            std::string banTime = permanent ? handler->GetSkyFireString(LANGUAGE_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
            handler->PSendSysMessage(LANGUAGE_BANINFO_HISTORYENTRY,
                fields[0].GetCString(), banTime.c_str(), active ? handler->GetSkyFireString(LANGUAGE_BANINFO_YES) : handler->GetSkyFireString(LANGUAGE_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
        }
    ...
    static bool HandleBanInfoCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!target)
        {
            ...
            if (!resultCharacter)
            {
                handler->PSendSysMessage(LANGUAGE_BANINFO_NOCHARACTER);
                return false;
            }
        ...
        if (!result)
        {
            handler->PSendSysMessage(LANGUAGE_CHARACTER_NOT_BANNED, name.c_str());
            return true;
        }

        handler->PSendSysMessage(LANGUAGE_BANINFO_BANHISTORY, name.c_str());
        do
        {
            Field* fields = result->Fetch();
            time_t unbanDate = time_t(fields[3].GetUInt32());
            ...
            std::string banTime = permanent ? handler->GetSkyFireString(LANGUAGE_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
            handler->PSendSysMessage(LANGUAGE_BANINFO_HISTORYENTRY,
                fields[0].GetCString(), banTime.c_str(), active ? handler->GetSkyFireString(LANGUAGE_BANINFO_YES) : handler->GetSkyFireString(LANGUAGE_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
        }
        while (result->NextRow());
    ...
    static bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!result)
        {
            handler->PSendSysMessage(LANGUAGE_BANINFO_NOIP);
            return true;
        }

        Field* fields = result->Fetch();
        bool permanent = !fields[6].GetUInt64();
        handler->PSendSysMessage(LANGUAGE_BANINFO_IPENTRY,
    ...
    {
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_DELETE_OLD_IP_BANS);
        ...
        if (filter.empty())
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SELECT_ACCOUNT_BANNED_ALL);
            result = LoginDatabase.Query(stmt);
        }
        else
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SELECT_ACCOUNT_BANNED_BY_USERNAME);
        ...
        if (!result)
        {
            handler->PSendSysMessage(LANGUAGE_BANLIST_NOACCOUNT);
            return true;
        }
    ...
    {
        handler->PSendSysMessage(LANGUAGE_BANLIST_MATCHINGACCOUNT);
        ...
        {
            handler->SendSysMessage(LANGUAGE_BANLIST_ACCOUNTS);
            handler->SendSysMessage(" ===============================================================================");
            handler->SendSysMessage(LANGUAGE_BANLIST_ACCOUNTS_HEADER);
            do
            {
                handler->SendSysMessage("-------------------------------------------------------------------------------");
                Field* fields = result->Fetch();
    ...
    static bool HandleBanListCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!result)
        {
            handler->PSendSysMessage(LANGUAGE_BANLIST_NOCHARACTER);
            return true;
        }

        handler->PSendSysMessage(LANGUAGE_BANLIST_MATCHINGCHARACTER);
        ...
        {
            handler->SendSysMessage(LANGUAGE_BANLIST_CHARACTERS);
            handler->SendSysMessage(" =============================================================================== ");
            handler->SendSysMessage(LANGUAGE_BANLIST_CHARACTERS_HEADER);
            do
            {
                handler->SendSysMessage("-------------------------------------------------------------------------------");
    ...
    static bool HandleBanListIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (filter.empty())
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SELECT_IP_BANNED_ALL);
            result = LoginDatabase.Query(stmt);
        }
        else
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SELECT_IP_BANNED_BY_IP);
            stmt->setString(0, filter);
            result = LoginDatabase.Query(stmt);
        }
        ...
        if (!result)
        {
            handler->PSendSysMessage(LANGUAGE_BANLIST_NOIP);
            return true;
        }

        handler->PSendSysMessage(LANGUAGE_BANLIST_MATCHINGIP);
        // Chat short output
        if (handler->GetSession())
        {
            do
        ...
        {
            handler->SendSysMessage(LANGUAGE_BANLIST_IPS);
            handler->SendSysMessage(" ===============================================================================");
            handler->SendSysMessage(LANGUAGE_BANLIST_IPS_HEADER);
            do
            {
                handler->SendSysMessage("-------------------------------------------------------------------------------");
                Field* fields = result->Fetch();
    ...
    static bool HandleUnBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        ...
        if (sWorld->RemoveBanAccount(mode, nameOrIP))
            handler->PSendSysMessage(LANGUAGE_UNBAN_UNBANNED, nameOrIP.c_str());
        else
            handler->PSendSysMessage(LANGUAGE_UNBAN_ERROR, nameOrIP.c_str());
