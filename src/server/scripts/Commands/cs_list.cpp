...
public:
    ...
    static bool HandleListCreatureCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!creatureId)
        {
            handler->PSendSysMessage(LANGUAGE_COMMAND_INVALIDCREATUREID, creatureId);
        ...
        if (result)
        {
            do
            {
                ...
                if (handler->GetSession())
                    handler->PSendSysMessage(LANGUAGE_CREATURE_LIST_CHAT, guid, guid, cInfo->Name.c_str(), x, y, z, mapId);
                else
                    handler->PSendSysMessage(LANGUAGE_CREATURE_LIST_CONSOLE, guid, cInfo->Name.c_str(), x, y, z, mapId);
            }
            while (result->NextRow());
        }

        handler->PSendSysMessage(LANGUAGE_COMMAND_LISTCREATUREMESSAGE, creatureId, creatureCount);
    ...
    static bool HandleListItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!itemId)
        {
            handler->PSendSysMessage(LANGUAGE_COMMAND_ITEMIDINVALID, itemId);
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (result)
        {
            do
            {
                ...
                handler->PSendSysMessage(LANGUAGE_ITEMLIST_SLOT, itemGuid, ownerName.c_str(), ownerGuid, ownerAccountId, itemPos);
            }
            while (result->NextRow());
        ...
        if (result)
        {
            do
            {
                ...
                handler->PSendSysMessage(LANGUAGE_ITEMLIST_MAIL, itemGuid, itemSenderName.c_str(), itemSender, itemSenderAccountId, itemReceiverName.c_str(), itemReceiver, itemReceiverAccount, itemPos);
            }
            while (result->NextRow());
        ...
        if (result)
        {
            do
            {
                ...
                handler->PSendSysMessage(LANGUAGE_ITEMLIST_AUCTION, itemGuid, ownerName.c_str(), owner, ownerAccountId, itemPos);
            }
            while (result->NextRow());
        }
        ...
        if (result)
        {
            do
            {
                ...
                handler->PSendSysMessage(LANGUAGE_ITEMLIST_GUILD, itemGuid, guildName.c_str(), guildGuid, itemPos);
            }
            while (result->NextRow());
        ...
        handler->PSendSysMessage(LANGUAGE_COMMAND_LISTITEMMESSAGE, itemId, inventoryCount + mailCount + auctionCount + guildCount, inventoryCount, mailCount, auctionCount, guildCount);
    ...
    static bool HandleListObjectCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!gameObjectId)
        {
            handler->PSendSysMessage(LANGUAGE_COMMAND_LISTOBJINVALIDID, gameObjectId);
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (result)
        {
            do
            {
                ...
                if (handler->GetSession())
                    handler->PSendSysMessage(LANGUAGE_GO_LIST_CHAT, guid, entry, guid, gInfo->name.c_str(), x, y, z, mapId);
                else
                    handler->PSendSysMessage(LANGUAGE_GO_LIST_CONSOLE, guid, gInfo->name.c_str(), x, y, z, mapId);
            }
            while (result->NextRow());
        }

        handler->PSendSysMessage(LANGUAGE_COMMAND_LISTOBJMESSAGE, gameObjectId, objectCount);
    ...
    static bool HandleListAurasCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        char const* talentStr = handler->GetSkyFireString(LANGUAGE_TALENT);
        char const* passiveStr = handler->GetSkyFireString(LANGUAGE_PASSIVE);

        Unit::AuraApplicationMap const& auras = unit->GetAppliedAuras();
        handler->PSendSysMessage(LANGUAGE_COMMAND_TARGET_LISTAURAS, auras.size());
        for (Unit::AuraApplicationMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        {
            ...
            handler->PSendSysMessage(LANGUAGE_COMMAND_TARGET_AURADETAIL, aura->GetId(), (handler->GetSession() ? ss_name.str().c_str() : name),
        ...
        for (uint16 i = 0; i < TOTAL_AURAS; ++i)
        {
            ...
            handler->PSendSysMessage(LANGUAGE_COMMAND_TARGET_LISTAURATYPE, auraList.size(), i);

            for (Unit::AuraEffectList::const_iterator itr = auraList.begin(); itr != auraList.end(); ++itr)
                handler->PSendSysMessage(LANGUAGE_COMMAND_TARGET_AURASIMPLE, (*itr)->GetId(), (*itr)->GetEffIndex(), (*itr)->GetAmount());
        }
