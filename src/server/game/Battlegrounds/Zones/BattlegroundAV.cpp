...
BattlegroundAV::BattlegroundAV()
{
    ...
    StartMessageIds[BG_STARTING_EVENT_FIRST]  = LANGUAGE_BG_AV_START_TWO_MINUTES;
    StartMessageIds[BG_STARTING_EVENT_SECOND] = LANGUAGE_BG_AV_START_ONE_MINUTE;
    StartMessageIds[BG_STARTING_EVENT_THIRD]  = LANGUAGE_BG_AV_START_HALF_MINUTE;
    StartMessageIds[BG_STARTING_EVENT_FOURTH] = LANGUAGE_BG_AV_HAS_BEGUN;
}
...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN][0])
    {
        ...
        if (creature)
            YellToAll(creature, GetSkyFireString(LANGUAGE_BG_AV_A_CAPTAIN_DEAD), LANGUAGE_UNIVERSAL);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN][0])
    {
        ...
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if (creature)
            YellToAll(creature, GetSkyFireString(LANGUAGE_BG_AV_H_CAPTAIN_DEAD), LANGUAGE_UNIVERSAL);
...
{ //note: to remove reinforcementpoints points must be negative, for adding reinforcements points must be positive
    ...
    if (points < 0)
    {
        ...
        {
            SendMessageToAll(teamindex == BG_TEAM_HORDE?LANGUAGE_BG_AV_H_NEAR_LOSE:LANGUAGE_BG_AV_A_NEAR_LOSE, teamindex == BG_TEAM_HORDE ? CHAT_MSG_BG_SYSTEM_HORDE : CHAT_MSG_BG_SYSTEM_ALLIANCE);
        ...
        for (uint8 i=0; i <= 1; i++)//0=alliance, 1=horde
        {
            ...
            else
            {
                if (i == 0)
                {
                    ...
                    if (creature)
                        YellToAll(creature, LANGUAGE_BG_AV_A_CAPTAIN_BUFF, LANGUAGE_COMMON);
                }
                else
                {
                    ...
                    if (creature)
                        YellToAll(creature, LANGUAGE_BG_AV_H_CAPTAIN_BUFF, LANGUAGE_ORCISH);
                }
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    char buf[256];
    if (IsTower(node))
        sprintf(buf, GetSkyFireString(LANGUAGE_BG_AV_TOWER_TAKEN), GetNodeName(node), (owner == ALLIANCE) ? GetSkyFireString(LANGUAGE_BG_AV_ALLY) : GetSkyFireString(LANGUAGE_BG_AV_HORDE));
    else
        sprintf(buf, GetSkyFireString(LANGUAGE_BG_AV_GRAVE_TAKEN), GetNodeName(node), (owner == ALLIANCE) ? GetSkyFireString(LANGUAGE_BG_AV_ALLY) :GetSkyFireString(LANGUAGE_BG_AV_HORDE));

    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
        YellToAll(creature, buf, LANGUAGE_UNIVERSAL);
}
...
//    }
    if (team == ALLIANCE || team == HORDE)
    {
        ...
        char buf[256];
        sprintf(buf, GetSkyFireString(LANGUAGE_BG_AV_MINE_TAKEN), GetSkyFireString((mine == AV_NORTH_MINE) ? LANGUAGE_BG_AV_MINE_NORTH : LANGUAGE_BG_AV_MINE_SOUTH), (team == ALLIANCE) ?  GetSkyFireString(LANGUAGE_BG_AV_ALLY) : GetSkyFireString(LANGUAGE_BG_AV_HORDE));
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if (creature)
            YellToAll(creature, buf, LANGUAGE_UNIVERSAL);
    }
    else
    {
        if (mine == AV_SOUTH_MINE) //i think this gets called all the time
        {
            if (Creature* creature = GetBGCreature(AV_CPLACE_MINE_S_3))
                YellToAll(creature, LANGUAGE_BG_AV_S_MINE_BOSS_CLAIMS, LANGUAGE_UNIVERSAL);
        }
    }
    return;
}
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
   ...
   //spawn new go :)
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, GetSkyFireString((IsTower(node)) ? LANGUAGE_BG_AV_TOWER_DEFENDED : LANGUAGE_BG_AV_GRAVE_DEFENDED), GetNodeName(node), (team == ALLIANCE) ?  GetSkyFireString(LANGUAGE_BG_AV_ALLY) : GetSkyFireString(LANGUAGE_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
        YellToAll(creature, buf, LANGUAGE_UNIVERSAL);
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, (IsTower(node)) ? GetSkyFireString(LANGUAGE_BG_AV_TOWER_ASSAULTED) : GetSkyFireString(LANGUAGE_BG_AV_GRAVE_ASSAULTED), GetNodeName(node), (team == ALLIANCE) ?  GetSkyFireString(LANGUAGE_BG_AV_ALLY) : GetSkyFireString(LANGUAGE_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
        YellToAll(creature, buf, LANGUAGE_UNIVERSAL);
...
const char* BattlegroundAV::GetNodeName(BG_AV_Nodes node)
{
    switch (node)
    {
        case BG_AV_NODES_FIRSTAID_STATION:  return GetSkyFireString(LANGUAGE_BG_AV_NODE_GRAVE_STORM_AID);
        case BG_AV_NODES_DUNBALDAR_SOUTH:   return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_DUN_S);
        case BG_AV_NODES_DUNBALDAR_NORTH:   return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_DUN_N);
        case BG_AV_NODES_STORMPIKE_GRAVE:   return GetSkyFireString(LANGUAGE_BG_AV_NODE_GRAVE_STORMPIKE);
        case BG_AV_NODES_ICEWING_BUNKER:    return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_ICEWING);
        case BG_AV_NODES_STONEHEART_GRAVE:  return GetSkyFireString(LANGUAGE_BG_AV_NODE_GRAVE_STONE);
        case BG_AV_NODES_STONEHEART_BUNKER: return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_STONE);
        case BG_AV_NODES_SNOWFALL_GRAVE:    return GetSkyFireString(LANGUAGE_BG_AV_NODE_GRAVE_SNOW);
        case BG_AV_NODES_ICEBLOOD_TOWER:    return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_ICE);
        case BG_AV_NODES_ICEBLOOD_GRAVE:    return GetSkyFireString(LANGUAGE_BG_AV_NODE_GRAVE_ICE);
        case BG_AV_NODES_TOWER_POINT:       return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_POINT);
        case BG_AV_NODES_FROSTWOLF_GRAVE:   return GetSkyFireString(LANGUAGE_BG_AV_NODE_GRAVE_FROST);
        case BG_AV_NODES_FROSTWOLF_ETOWER:  return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_FROST_E);
        case BG_AV_NODES_FROSTWOLF_WTOWER:  return GetSkyFireString(LANGUAGE_BG_AV_NODE_TOWER_FROST_W);
        case BG_AV_NODES_FROSTWOLF_HUT:     return GetSkyFireString(LANGUAGE_BG_AV_NODE_GRAVE_FROST_HUT);
        default:
