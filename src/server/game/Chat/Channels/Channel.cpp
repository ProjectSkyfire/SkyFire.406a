 ...
 : m_announce(true), m_ownership(true), m_name(name), m_password(""), m_flags(0), m_channelId(channel_id), m_ownerGUID(0), m_Team(Team)
{
    ...
    else                                                    // it's custom channel
    {
        ...
        {
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SELECT_CHANNEL);
            ...
            {
                stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_CHANNEL);
                stmt->setString(0, name);
                stmt->setUInt32(1, m_Team);
...
void Channel::UpdateChannelInDB() const
{
    if (m_IsSaved)
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_CHANNEL);
...
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_CHANNEL_USAGE);
...
void Channel::CleanOldChannelsInDB()
{
    ...
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_OLD_CHANNELS);
