...
void Guild::EventLogEntry::SaveToDB(SQLTransaction& trans) const
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_EVENTLOG);
    stmt->setUInt32(0, m_guildId);
    stmt->setUInt32(1, m_guid);
    CharacterDatabase.ExecuteOrAppend(trans, stmt);

    uint8 index = 0;
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_EVENTLOG);
...
void Guild::BankEventLogEntry::SaveToDB(SQLTransaction& trans) const
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_EVENTLOG);
    stmt->setUInt32(  index, m_guildId);
    stmt->setUInt32(++index, m_guid);
    ...
    index = 0;
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_BANK_EVENTLOG);
    stmt->setUInt32(  index, m_guildId);
    stmt->setUInt32(++index, m_guid);
    stmt->setUInt8 (++index, m_bankTabId);
    stmt->setUInt8 (++index, uint8(m_eventType));
...
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_RANK);
    stmt->setUInt32(0, m_guildId);
...
void Guild::RankInfo::SetName(const std::string& name)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_RANK_NAME);
...
void Guild::RankInfo::SetRights(uint32 rights)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_RANK_RIGHTS);
...
void Guild::RankInfo::SetBankMoneyPerDay(uint32 money)
{
    ...
    PreparedStatement* stmt = NULL;
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_RANK_BANK_MONEY);
    stmt->setUInt32(0, money);
    stmt->setUInt8 (1, m_rankId);
    stmt->setUInt32(2, m_guildId);
    CharacterDatabase.Execute(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_RANK_BANK_RESET_TIME);
    stmt->setUInt32(0, m_guildId);
    stmt->setUInt8 (1, m_rankId);
    CharacterDatabase.Execute(stmt);
}
...
void Guild::RankInfo::SetBankTabSlotsAndRights(uint8 tabId, GuildBankRightsAndSlots rightsAndSlots, bool saveToDB)
{
    ...
    if (saveToDB)
    {
        ...
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_RIGHT);
        stmt->setUInt32(0, m_guildId);
        stmt->setUInt8 (1, tabId);
        stmt->setUInt8 (2, m_rankId);
        CharacterDatabase.Execute(stmt);

        stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_BANK_RIGHT);
        stmt->setUInt32(0, m_guildId);
        stmt->setUInt8 (1, tabId);
        stmt->setUInt8 (2, m_rankId);
        ...
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_RANK_BANK_TIME0 + tabId);
        stmt->setUInt32(0, m_guildId);
        stmt->setUInt8 (1, m_rankId);
        CharacterDatabase.Execute(stmt);
    }
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    if (!pItem->LoadFromDB(itemGuid, 0, fields, itemEntry))
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_NONEXISTENT_GUILD_BANK_ITEM);
        stmt->setUInt32(0, m_guildId);
...
void Guild::BankTab::SetInfo(const std::string& name, const std::string& icon)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_BANK_TAB_INFO);
    stmt->setString(0, m_name);
...
void Guild::BankTab::SetText(const std::string& text)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_BANK_TAB_TEXT);
...
bool Guild::BankTab::SetItem(SQLTransaction& trans, uint8 slotId, Item* pItem)
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_ITEM);
    stmt->setUInt32(0, m_guildId);
    stmt->setUInt8 (1, m_tabId);
    stmt->setUInt8 (2, slotId);
    CharacterDatabase.ExecuteOrAppend(trans, stmt);
    ...
    if (pItem)
    {
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_BANK_ITEM);
        stmt->setUInt32(0, m_guildId);
        stmt->setUInt8 (1, m_tabId);
        stmt->setUInt8 (2, slotId);
...
void Guild::Member::SetPublicNote(const std::string& publicNote)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_MEMBER_PNOTE);
...
void Guild::Member::SetOfficerNote(const std::string& officerNote)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_MEMBER_OFFNOTE);
...
void Guild::Member::ChangeRank(uint8 newRank)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_MEMBER_RANK);
...
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_MEMBER);
    stmt->setUInt32(0, m_guildId);
    stmt->setUInt32(1, GUID_LOPART(m_guid));
    stmt->setUInt8 (2, m_rankId);
...
void Guild::Member::DecreaseBankRemainingValue(SQLTransaction& trans, uint8 tabId, uint32 amount)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(
        ...
        CHAR_UPDATE_GUILD_MEMBER_BANK_REM_SLOTS0 + tabId);
...
uint32 Guild::Member::GetBankRemainingValue(uint8 tabId, const Guild* guild) const
{
    ...
    if (curTime > m_bankRemaining[tabId].resetTime + 24 * HOUR / MINUTE)
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(
            ...
            CHAR_UPDATE_GUILD_MEMBER_BANK_TIME_REM_SLOTS0 + tabId);
...
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_EMBLEM_INFO);
...
bool Guild::Create(Player* pLeader, const std::string& name)
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_MEMBERS);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);

    uint8 index = 0;
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD);
...
void Guild::Disband()
{
    ...
    PreparedStatement* stmt = NULL;
    SQLTransaction trans = CharacterDatabase.BeginTransaction();
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_RANKS);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_TABS);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_ITEMS);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_RIGHTS);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_EVENTLOGS);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_EVENTLOGS);
    stmt->setUInt32(0, m_id);
    trans->Append(stmt);
...
void Guild::HandleSetMOTD(WorldSession* session, const std::string& motd)
{
    ...
    else
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_MOTD);
...
void Guild::HandleSetInfo(WorldSession* session, const std::string& info)
{
    ...
    else
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_INFO);
...
void Guild::HandleRemoveRank(WorldSession* session, uint8 rankId)
{
    ...
    else
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_RIGHTS_FOR_RANK);
        stmt->setUInt32(0, m_id);
        stmt->setUInt8 (1, rankId);
        CharacterDatabase.Execute(stmt);
        // Delete rank
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_LOWEST_RANK);
        stmt->setUInt32(0, m_id);
        stmt->setUInt8 (1, rankId);
        CharacterDatabase.Execute(stmt);
...
bool Guild::AddMember(uint64 guid, uint8 rankId)
{
    ...
    else
    {
        ...
        bool ok = false;
        // Player must exist
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SELECT_CHAR_DATA_FOR_GUILD);
...
bool Guild::_CreateNewBankTab()
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GUILD_BANK_TAB);
    stmt->setUInt32(0, m_id);
    stmt->setUInt8 (1, tabId);
    trans->Append(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_BANK_TAB);
    stmt->setUInt32(0, m_id);
    stmt->setUInt8 (1, tabId);
    trans->Append(stmt);
...
void Guild::_CreateRank(const std::string& name, uint32 rights)
{
    ...
    for (uint8 i = 0; i < _GetPurchasedTabsSize(); ++i)
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_BANK_RIGHT_DEFAULT);
        stmt->setUInt32(0, m_id);
        stmt->setUInt8 (1, i);
...
bool Guild::_ModifyBankMoney(SQLTransaction& trans, uint64 amount, bool add)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_BANK_MONEY);
...
void Guild::_SetLeaderGUID(Member* pLeader)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_LEADER);
...
void Guild::SaveXP()
{
    if (getMSTime() - m_lastXPSave >= 60000) // 1 minute. Hardcoded value for now
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GUILD_SAVE_XP);
...
void Guild::AddGuildNews(uint32 type, uint64 source_guild, int value1, int value2, int flags)
{
    ...
    SQLTransaction trans = CharacterDatabase.BeginTransaction();
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_GUILD_NEWS);
