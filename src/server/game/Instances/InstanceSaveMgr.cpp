...
void InstanceSave::SaveToDB()
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_INSTANCE_SAVE);
...
void InstanceSaveManager::LoadResetTimes()
{
    ...
    if (result)
    {
        ...
        if (PreparedQueryResult result2 = CharacterDatabase.Query(CharacterDatabase.GetPreparedStatement(CHAR_SELECT_MAX_CREATURE_RESPAWNS)))
        {
            do
            {
