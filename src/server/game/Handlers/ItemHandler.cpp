...
void WorldSession::HandleRequestHotFix(WorldPacket& recvData)
{
    uint32 count, type;
    recvData >> count >> type;
...
void WorldSession::HandleBuyItemInSlotOpcode(WorldPacket& recvData)
{
    ...
    recvData >> vendorguid >> unk >> item  >> slot >> count >> bagguid >> bagslot;
    ...
    for (uint32 slot = 0; slot < itemCount; ++slot)
    {
        if (VendorItem const* item = items->GetItem(slot))
        {
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(item->item))
            {
				...
				// uncomment when repItems flags are fixed.
...
void WorldSession::HandleReforgeItem(WorldPacket& recvData)
{
