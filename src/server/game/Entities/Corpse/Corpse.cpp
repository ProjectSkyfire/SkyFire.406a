...
void Corpse::SaveToDB()
{
    ...
    uint8 index = 0;
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INSERT_CORPSE);
...
void Corpse::DeleteFromDB(SQLTransaction& trans)
{
    ...
    {
        // Only specific bones
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_CORPSE);
        stmt->setUInt32(0, GetGUIDLow());
    }
    else
    {
        // all corpses (not bones)
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_PLAYER_CORPSES);
