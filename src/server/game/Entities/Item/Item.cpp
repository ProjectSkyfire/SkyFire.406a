...
void Item::SaveToDB(SQLTransaction& trans)
{
    ...
    switch (uState)
    {
        ...
        {
            uint8 index = 0;
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(uState == ITEM_NEW ? CHAR_REPLACE_ITEM_INSTANCE : CHAR_UPDATE_ITEM_INSTANCE);
        ...
        case ITEM_REMOVED:
        {
            ...
            {
                stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_GIFT);
                stmt->setUInt32(0, guid);
                trans->Append(stmt);
            }
...
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_INVENTORY_ITEM);
    stmt->setUInt32(0, itemGuid);
    trans->Append(stmt);
}
...
void Item::ClearSoulboundTradeable(Player* currentOwner)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DELETE_ITEM_BOP_TRADE);
