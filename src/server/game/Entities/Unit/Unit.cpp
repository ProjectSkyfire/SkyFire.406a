...
void Unit::CalcHealAbsorb(Unit* victim, const SpellInfo *healSpell, uint32 &healAmount, uint32 &absorb)
{
    ...
    if (existExpired)
    {
        for (AuraEffectList::const_iterator i = vHealAbsorb.begin(); i != vHealAbsorb.end();)
        {
            ...
            ++i;
    		if (auraEff->GetId() != 73975)//Ignore necrotic strike
			//undefined auras            
    ...
    healAmount = RemainingHeal;
      ...
      if (Aura * aur = GetAura(73975))//Necrotic strike remover
      {
			//Necrotic strike
			int32 zabsorb = GetAbsorbHeal();//Get initial absorb value
			int32 subtract = zabsorb - absorb;//Define subtraction
