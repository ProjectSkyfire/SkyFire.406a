...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_UPD_EXPIREDACCPREM, "UPDATE account_premium SET active = 0 WHERE active = 1 AND unsetdate<>setdate AND unsetdate<=UNIX_TIMESTAMP()", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_EXPIREDACCPREMGM, "DELETE FROM account_access WHERE id IN (SELECT id FROM account_premium WHERE active = 0)", CONNECTION_ASYNC);
}
