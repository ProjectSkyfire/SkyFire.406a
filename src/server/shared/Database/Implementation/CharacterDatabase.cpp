...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(CHAR_ADD_GUILD, "INSERT INTO guild (guildid, name, leaderguid, info, motd, createdate, EmblemStyle, EmblemColor, BorderStyle, BorderColor, BackgroundColor, BankMoney, totalXP, todayXP, XPCap, level) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_SET_GUILD_SAVE_XP, "UPDATE guild SET totalXP = ?, todayXP = ?, XPCap = ?, level = ? WHERE guildid = ?", CONNECTION_ASYNC); // 1: uint64, 2,3: uint32
    // 0-5: uint32
